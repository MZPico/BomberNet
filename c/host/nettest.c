...
#include "uc.h"
int nettest(void) {
  net_status_t s; char code[5]; uint8_t slot, keys[4], r; uint16_t seed, start, avail;
  net_detect();
  printf("net_device %u\n", net_device);
  r = net_status(&s); printf("status r=%u state=%u\n", r, s.state);
  r = net_create(0x0101, (const uint8_t *)"\x01\x02", 2, code, &slot); printf("create r=%u code=%s slot=%u\n", r, code, slot);
